    ...
    public static Specification<Event> withFilter(EventFilterDTO filter) {
        return (root, query, criteriaBuilder) -> {
            ...
            if (filter.getKeyword() != null && !filter.getKeyword().isEmpty()) {
                ...
                Predicate inTitle = criteriaBuilder.like(criteriaBuilder.lower(root.get("title")), pattern);
                Predicate inDescription = criteriaBuilder.like(criteriaBuilder.lower(root.get("description")), pattern);
                Predicate inLocation = criteriaBuilder.like(criteriaBuilder.lower(root.get("location")), pattern);
                predicates.add(criteriaBuilder.or(inTitle, inDescription, inLocation));
            }

            // Точный поиск по location
            ...
            if (filter.getEventTypeId() != null) {
                predicates.add(criteriaBuilder.equal(
                        root.get("eventType").get("id"), filter.getEventTypeId()
                ));
            }
